...
1. Cloud data reaching the Prior layer is corrupted or spoofed in transit or at rest.
   ...
   * Disposition: New payload (Interface 1's restriction/geometry content) riding on a **Pre-existing** baseline assumption. Data integrity and authenticity validation at the point cloud data enters the vehicle is assumed to already exist as baseline connectivity infrastructure, the same assumption carried in the pothole-detection project. This use case adds no new requirement here and relies on that checking function being in place rather than introducing or re-verifying it.
...
2. Restriction data served to the Prior layer is stale - a new closure, restriction change, or lifted restriction not yet reflected.
   * Category: SOTIF
   * Rationale: The Cloud Prior Layer serves exactly the data it holds, functioning to spec; the insufficiency is the system's reliance on non-real-time data as if current - a known, foreseeable performance limitation, not a malfunction. This is distinct from Items 5 and 6 below: staleness is an inherent latency property of any non-real-time data source, whether delivered by prefetch or continuous streaming, not a cloud-backend content error, so it remains a vehicle-side SOTIF item rather than an out-of-scope backend issue.
   * Disposition: Behavior Planner's decision tree includes a live-perception recheck (node G, decision_tree.md) that measures actual clear length immediately before commit, independent of the cloud prior data - this catches a stale entry that misstates *physical* clearance. Real-time detection and localization of street parking-restriction signage from a moving vehicle is a demonstrated capability in current perception research - for example, Chau, Jin, Li, Hu, and Cheng demonstrate parking-sign and parking-symbol detection at 163 and 88 frames per second respectively using YOLOv5, at production-relevant accuracy (IJCAI 2022 AI for Autonomous Driving Workshop) - so a live cross-check against legal permissibility is not physically out of reach. What is not yet demonstrated at the same maturity, per that same paper, is full extraction of structured restriction semantics (time windows, permit exceptions) from sign text and delivery of that as an actionable signal to a planner - the authors explicitly flag that step as future work, not a solved, integrated capability. World Model Builder now has a real-time Perception-based restriction classification channel (interface_control_document.md, Baseline Dependency A). Its trigger condition is Interface 1's `stream_health_status` degrading, not content staleness of an otherwise-healthy stream - this item's scenario (the stream is current and flowing, but the underlying database content is itself behind a very recent restriction change) does not cross that trigger, so it remains substantively unmitigated even with the new channel in place. What the new channel closes is a narrower, different case - see Items 3a and 3b. Whether the channel's trigger condition should be broadened to also run as a continuous cross-check against a healthy-but-potentially-stale stream, which would actually close this item's scenario, is a genuine open design question - see Items Not Yet Classifiable.

3a. The restriction-data stream has not yet delivered any tile for the vehicle's current position by the time the anticipatory-search trigger is reached (stream never established before it was needed), due to a network or hardware fault.
    * Category: HARA
    * Rationale: An external communication path fails to perform its specified function.
    * Disposition: New, and currently unmitigated. interface_control_document.md Open Item OI-6 names this case explicitly but leaves the resulting behavior undefined - there is no data to have degraded from, so it is not clear the Perception fallback channel (Baseline Dependency A), which activates on a stream degrading, applies to a stream that never started. This differs from Item 2: node G's live recheck only ever validates physical clearance, and this case removes restriction-class data entirely, not just ages it.

3b. An established restriction-data stream is lost or degrades mid-search, due to a network or hardware fault interrupting the connection.
    * Category: HARA
    * Rationale: An external communication path fails to perform its specified function.
    * Disposition: New, and now partially mitigated. interface_control_document.md's `stream_health_status` field (Interface 1) detects this condition, and World Model Builder falls back to the Perception-based restriction classification channel (Baseline Dependency A) once the degradation persists past the bound in Open Item OI-1. The fallback is coarser than the original stream - PERMITTED/RESTRICTED/UNKNOWN classification only, not full time-window semantics - and the exact hysteresis timing before switching tiers is still open (OI-12), but this is no longer a total loss of restriction data the way it was before the ICD update.

4. The restriction-data stream degrades or is lost because the vehicle is operating in a low-connectivity pocket of the ODD, a foreseeable condition in dense urban cores.
   * Category: SOTIF
   ...
   * Disposition: New, and now partially mitigated for the same reason as Item 3b - a low-connectivity pocket causing sustained STREAM_LOST now degrades gracefully into Perception-based classification (Baseline Dependency A) rather than leaving zero restriction data, subject to that channel's own coarse-classification ceiling (the same detection-yes/semantics-not-yet limit noted in Item 2) and the still-open OI-1/OI-12 timing bounds. If the connectivity gap prevents the stream from ever establishing in the first place, Item 3a's unmitigated case applies instead.
...
5. Source restriction data is itself incorrect at authoring time - for example, a safety-critical restriction mislabeled as regulatory-tier.
   ...
   * Rationale: This is a defect in the cloud backend's own map-authoring and data-quality process, not a malfunction or performance limitation of any vehicle-side or transport component - every vehicle-side element applies the data exactly as delivered. This is not classified as a vehicle-side SOTIF item; it is the cloud backend's responsibility to resolve through its own authoring and QA controls.
   * Disposition: Not this use case's content to mitigate. Retained in this list for completeness, since the vehicle-side consequence (a wrong commit decision) is real even though the fix lives outside this use case's boundary. The Perception-based restriction classification channel (Baseline Dependency A) does not backstop this item either, for the same reason it does not backstop Item 2: its trigger condition is stream-health degradation, and this scenario is a healthy stream delivering incorrect content - the trigger never fires.
...
------------------------------
## Perception Restriction-Classification Fallback (Baseline Dependency A)

44. The Perception restriction-classification channel confidently returns PERMITTED for a candidate that is actually restricted - a sign misread, curb paint misclassified under adverse lighting - while operating within its specified detection capability.
    * Category: SOTIF
    * Rationale: The classifier performs to its demonstrated capability [1]; the insufficiency is that capability being wrong in the dangerous direction for this specific scenario, not a malfunction. Distinct from the channel's designed-in safe direction: UNKNOWN is already treated identically to RESTRICTED (interface_control_document.md, Open Item OI-9's tier-collapse precedent), so this item is specifically the confident-and-wrong case, not the low-confidence case.
    * Disposition: New - this channel and its failure modes did not exist before the ICD update introducing it. Unmitigated; no independent check currently verifies this channel's classification before it is substituted into `restriction_class_channel`.

45. The `stream_health_status` field itself reports OK while the underlying restriction-data stream is actually degraded or delivering corrupted content.
    * Category: HARA
    * Rationale: A malfunction of the health-reporting mechanism itself, distinct from a malfunction of the data it is meant to describe - a false-healthy reading suppresses the fallback rather than triggering it.
    * Disposition: New - this field did not exist before the ICD update introducing it. Unmitigated; nothing currently cross-checks `stream_health_status` against the actual freshness or correctness of the data it reports on.
...
* Whether the Perception restriction-classification channel's trigger condition (Baseline Dependency A, interface_control_document.md) should be broadened from pure stream-health degradation to also run as a continuous cross-check against a healthy-but-potentially-stale stream - the only way it would actually close Items 2 and 5's scenarios rather than only Items 3b and 4's.
