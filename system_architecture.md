...
This document defines the functional block structure of the system and the primary data flows between blocks. It establishes the architectural context that the Interface Control Document later builds on to define each interface's content in detail.
...
* **Cloud Prior Layer** - Maintains and serves the time-of-day-aware curb restriction dataset and static map geometry, as a prefetched static grid layer.
* **Cloud Routing & POI Service** - Serves fallback-hub/POI location data directly to Trip Manager.
